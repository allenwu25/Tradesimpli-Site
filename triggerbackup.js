...
exports = async function() {
    const userscollection = context.services.get("StockTrader").db("test").collection("users");
    const portfoliocollection = context.services.get('StockTrader').db("test").collection("portfolios");
    const collectionstocks = context.services.get('StockTrader').db("test").collection("stocks");
    const accountcollection = context.services.get("StockTrader").db("test").collection("accountvalues");
    ...
    for (let i = 0; i < users.length; i++) {
        ...
        var portfolio = await portfoliocollection.findOne({user: userid})
        ...
        if (userstocks.length > 0) {
            ...
            var pricedata = await context.http.get({ url: url, headers: {"Content-Type": ["application/json"]} })
            pricedata = EJSON.parse(pricedata.body.text())
            ...
            totalvalue = parseFloat(calculatetotal(allthestocks)) + parseFloat(cash.toFixed(2))
